...
// All four mutations below suppress the interceptor's automatic error toast — every
// caller handles its own errors via shared/utils/admin-mutation-error.util.ts, since a
// 409 INVALID_STATUS_TRANSITION and a 400 VALIDATION_FAILED on these screens must never
// be a toast (inline banner / field error instead — see admin-mutation-error.util.ts).
